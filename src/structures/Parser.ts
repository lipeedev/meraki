...
export class Parser {
  ...
  private parseVariableDeclaration(token: Token) {
    ...
    if (nextTokenAfterEquals.type === TokenType.Identifier) {
      if (this.isModuleAccessField(nextTokenAfterEquals)) {
  ...
  private isVariableAssignment(token: Token) {
    ...
    return token.type === TokenType.Identifier && nextToken.type === TokenType.Equals
      && previousToken.type !== TokenType.Identifier && previousToken.value !== keywords.variableDeclaration;
  }
