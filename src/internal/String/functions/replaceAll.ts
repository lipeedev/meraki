...
export function replaceAll({ args, line, column, variables }: ModuleFunctionCallParams) {
    if (args.length !== argsSize) {
        return sendError({
            message: `replaceAll() takes exactly ${argsSize} arguments (${args.length} given)`,
            line,
            column
        });
    }
    ...
    if (args[0].type !== TokenType.String && args[0].type !== TokenType.Identifier) {
        return sendError({
            message: 'replaceAll() argument 1 must be a string',
            line: args[0].line,
    ...
    if (args[1].type !== TokenType.String && args[1].type !== TokenType.Identifier) {
        return sendError({
            message: 'replaceAll() argument 2 must be a string',
            line: args[1].line,
    ...
    if (args[2].type !== TokenType.String && args[2].type !== TokenType.Identifier) {
        return sendError({
            message: 'replaceAll() argument 3 must be a string',
            line: args[2].line,
    ...
    return {
        returnValue: string.replaceAll(search, replace),
