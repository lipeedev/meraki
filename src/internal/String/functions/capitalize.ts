...
export function capitalize({ args, line, column, variables, functionsReturn }: ModuleFunctionCallParams) {
    let value;

    if (args.length !== argsSize) {
        return sendError({
            message: `capitalize() takes exactly ${argsSize} argument (${args.length} given)`,
            line,
            column
        });
    }
    ...
    if (args[0].type !== TokenType.String && args[0].type !== TokenType.Identifier) {
        return sendError({
            message: 'capitalize() argument must be a string',
            line: args[0].line,
    ...
    return {
        returnValue: value[0].toUpperCase() + value.slice(1).toLowerCase(),
