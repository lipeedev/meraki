...
export function length({ args, line, column, variables, functionsReturn }: ModuleFunctionCallParams) {
    let value;

    if (args.length !== argsSize) {
        return sendError({
            message: `length() takes exactly ${argsSize} argument (${args.length} given)`,
            line,
            column
        });
    }
    ...
    if (args[0].type !== TokenType.String && args[0].type !== TokenType.Identifier) {
        return sendError({
            message: 'length() argument must be a string',
            line: args[0].line,
